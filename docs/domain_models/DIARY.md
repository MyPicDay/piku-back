...
- 작성자가 아닌 사용자는 `PRIVATE` 일기의 **대표 이미지 1장만** 반환받을 수 있다.
- 이 대표 이미지는 **캘린더 커버 이미지, 목록 미리보기, 요약 카드 썸네일** 용도로만 사용한다.
- 작성자가 아닌 사용자는 `PRIVATE` 일기의 본문, 비대표 이미지, 피드용 상세 정보에 접근할 수 없다.
- `PRIVATE` 일기는 **어떠한 경우에도 피드 추천 후보에 포함되어서는 안 된다.**
- 위 규칙은 신규 피드 생성뿐 아니라 **캐시 복원, 재정렬, 재추천** 과정에도 동일하게 적용된다.

#### 정책 해석 메모

- `대표 이미지 1장 반환 가능`은 `PRIVATE`가 공개 일기라는 뜻이 아니라, 썸네일 정책의 예외 허용이다.
- 피드 도메인은 `DiaryVisibility`를 자체 해석하지 않고, `Diary` 도메인이 정의한 공개 범위 정책을 준수해야 한다.
...
- `PRIVATE` 일기의 대표 이미지는 작성자가 아닌 사용자에게 반환될 수 있으나, 이는 썸네일 예외 정책이며 피드 추천 허용을 의미하지 않는다.
