...
- 사용자가 이미지 생성을 선택한 일기에 시각적 표현을 제공한다.
...
- 일기 작성은 AI 이미지 생성 없이 시작하고 완료할 수 있다. 사용자가 AI 이미지를 선택한 경우에만 생성 결과를 일기에 연결하며, 생성 실패는 이미지 생성 시도의 실패로 처리하고 텍스트 일기 등록 가능 여부와 결합하지 않는다.
