...
현재 생성 흐름은 요청 통계 기록, 쿼터 소비, 사용자 아바타 참조 준비, 외부 AI 호출, 생성 이미지 저장, 표시 URL 해석과 생성 이력 기록 순서로 수행한다.
...
- User 공개 조회 계약은 사용자가 선택한 아바타 저장 참조만 제공한다.
- Creative Application은 저장 참조를 생성 입력용 canonical 참조로 번역한다.
- User 조회와 Object Storage 로드는 서로 다른 Out Port와 Adapter가 담당한다.
...
- 일일 생성 한도 초과, 참조 이미지 부재, 외부 생성 실패, 저장 실패와 생성 이력 부재는 기술 중립 Creative 오류 코드로 표현한다.
