...
- 이미지 첨부는 선택이며 AI 생성 이미지와 사용자 업로드 이미지를 함께 사용할 수 있다. Diary는 기록에 사용할 이미지의 연결과 표시를 소유하지만 이미지 생성 과정과 생성 이력은 소유하지 않는다.
...
- 현재 일기는 본문과 선택적인 AI 생성·사용자 업로드 이미지를 기록하며 감정 값이나 감정 통계를 별도로 보유하지 않는다.
- 삭제된 일기는 사용자-facing 조회와 사용자 기록 집계에서 제외된다. Admin의 누적 생성 이벤트 지표는 삭제 여부와 무관하게 생성 사실을 집계한다.
...
- 사진이 있으면 `photoOrder`는 `0`부터 중복이나 누락 없이 이어져야 하며, 순서 `0`인 사진을 대표 사진으로 지정한다.
