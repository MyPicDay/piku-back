...
Social 도메인은 **사용자 간의 친구 관계와 댓글, 좋아요 같은 컨텐츠 상호작용을 모델링하고 관리**하는 도메인입니다.
...
- 익명 일기의 댓글을 일기 작성자와 루트 댓글 작성자 사이의 비공개 대화로 보호한다.
...
- `FriendRequest`가 수락되면 요청의 생명주기가 끝나고 새 `Friend` Aggregate가 생성된다.
- `Comment`는 `parent` 자기 참조 관계로 계층 구조(댓글 / 답글)를 지원한다.
- 하나의 사용자는 하나의 일기에 활성 좋아요를 하나만 가질 수 있다.
...
- 친구 요청은 대기 상태 자체가 도메인 의미이므로 수락·거절·철회 시 생명주기를 종료한다.
...
- `createdAt` : LocalDateTime. 댓글 생성 일시
- `updatedAt` : LocalDateTime. 댓글 수정 일시
...
- 댓글 수 집계는 삭제되지 않은 댓글만 포함한다. 삭제된 루트 댓글이 활성 답글 때문에 목록에 placeholder로 남더라도, 삭제된 루트 댓글 자체는 댓글 수에 포함하지 않고 활성 답글만 포함한다.
...
- `createdAt` : LocalDateTime. 좋아요 생성 일시
...
- `userId`와 `diaryId`의 조합은 하나의 좋아요 관계를 식별한다.
...
- 비친구는 일기 식별자를 알고 있더라도 숨겨진 일기의 내용과 상호작용 정보를 조회할 수 없다.
- 댓글 또는 부모 댓글 식별자를 직접 아는 경우에도 숨겨진 일기의 내용이 노출되어서는 안 된다.

### PRIVATE 일기 상호작용 정책

- `PRIVATE` 일기의 댓글과 좋아요는 작성자 본인만 접근할 수 있다.
- 비소유자는 `PRIVATE` 일기의 댓글 목록, 답글 목록, 댓글 수, 좋아요 상태, 좋아요 수에 접근할 수 없다.
- 댓글 식별자를 직접 아는 경우에도 숨겨진 일기의 본문과 상호작용 정보가 노출되어서는 안 된다.

### ANONYMOUS 일기 댓글 정책

- 익명 일기를 볼 수 있는 인증 사용자는 새 루트 댓글을 작성할 수 있으며, 각 루트 댓글은 일기 작성자와 루트 댓글 작성자 사이의 비공개 대화 시작점이 된다.
- 일기 작성자는 익명 일기의 모든 루트 댓글과 답글 원문을 볼 수 있다.
- 루트 댓글 작성자는 자신이 작성한 루트 댓글과 그 아래의 모든 답글 원문을 볼 수 있다.
- 답글은 일기 작성자와 해당 루트 댓글 작성자만 작성할 수 있다.
- 일기 작성자와 해당 루트 댓글 작성자가 아닌 사용자는 댓글과 답글의 존재만 `비공개 댓글`, `비공개 답글` placeholder로 확인한다.
- 댓글 목록, 답글 목록과 댓글 수는 원문 열람 권한과 관계없이 전체 활성 댓글을 기준으로 유지한다.
- 익명 댓글의 실제 `userId`, 닉네임과 아바타는 일기 작성자, 루트 댓글 작성자와 댓글 작성자 본인에게도 노출하지 않는다.
- 댓글 작성자 본인 여부와 허용 액션은 서버가 계산한 답글·수정·삭제 권한으로 표현한다.
- 수정과 삭제는 실제 댓글 작성자만 수행할 수 있으며, placeholder로만 보이는 댓글에는 어떤 액션도 허용하지 않는다.
- 비로그인 사용자는 댓글과 답글 원문을 볼 수 없고 댓글 작성, 답글, 수정과 삭제를 수행할 수 없다.
