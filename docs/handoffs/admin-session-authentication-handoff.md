...
- OTP 검증 성공 후 `authenticated`와 관리자 요약 정보로 인증 완료 상태를 갱신한다.
...
6. 응답의 `authenticated`가 true이면 관리자 요약 정보를 저장하고 보호 화면으로 이동한다.

로그인 응답의 관리자 요약에는 로그인 아이디, 닉네임, 이메일, 등급이 포함된다. OTP 검증 전에는 보호 API를 호출할 수 없다.
...
9. 응답의 `authenticated`가 true이면 관리자 요약 정보를 저장하고 보호 화면으로 이동한다.

로그인 아이디와 패스워드는 하나의 요청으로 설정한다. OTP 등록 응답의 발급자, 계정명, 등록 URI, 수동 입력 키는 OTP 등록 화면에서만 사용하며 인증 완료 후 보관하지 않는다.
...
| `POST /api/admin/auth/temporary-login` | 사전 세션, CSRF, Origin | `nextStep`, 이메일, 닉네임, 등급 반영 |
...
| `POST /api/admin/auth/onboarding/otp/verify` | 온보딩 OTP 검증 단계, CSRF, Origin | 인증 완료 상태와 관리자 요약 반영 |
| `POST /api/admin/auth/login` | 사전 세션, CSRF, Origin | `nextStep`에 따라 OTP 입력 화면으로 이동 |
| `POST /api/admin/auth/otp/verify` | 로그인 OTP 검증 단계, CSRF, Origin | 인증 완료 상태와 관리자 요약 반영 |
