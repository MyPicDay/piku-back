...
class AdminExceptionHandlerTest {
	...
	private final RecordAdminSecurityEventUseCase recordAdminSecurityEventUseCase =
			mock(RecordAdminSecurityEventUseCase.class);
	...
	void mapsDataStoreFailureToServiceUnavailable() {
		...
		assertThat(response.getBody().getType())
				.isEqualTo(AdminProblemType.SESSION_STORE_UNAVAILABLE.type());
		...
		then(recordAdminSecurityEventUseCase).should().recordSessionStoreUnavailable();
	}

	private AdminExceptionHandler handler() {
		return new AdminExceptionHandler(
				new ProblemDetailFactory(),
				recordAdminSecurityEventUseCase);
	}
}
