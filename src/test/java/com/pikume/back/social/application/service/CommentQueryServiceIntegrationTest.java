...
class CommentQueryServiceIntegrationTest extends AbstractJpaQueryCountIntegrationTest {
	...
	void rootCommentQueryCountStaysBounded() {
		...
		long oneItemQueries = measurePreparedStatements(() ->
				commentQueryService.queryRootCommentPage(diary.getId(), PageQuery.of(0, 1), owner.getId()));
		long threeItemQueries = measurePreparedStatements(() ->
				commentQueryService.queryRootCommentPage(diary.getId(), PageQuery.of(0, 3), owner.getId()));
	...
	void replyQueryCountStaysBounded() {
		...
		long oneItemQueries = measurePreparedStatements(() ->
				commentQueryService.queryReplyPage(parent.getId(), PageQuery.of(0, 1), owner.getId()));
		long threeItemQueries = measurePreparedStatements(() ->
				commentQueryService.queryReplyPage(parent.getId(), PageQuery.of(0, 3), owner.getId()));
	...
	void commentCountsExcludeDeletedComments() {
		...
		long count = commentQueryService.queryActiveCommentCount(owner.getId(), diary.getId());
		Map<Long, Long> countsByDiaryId = commentQueryService.queryCommentCounts(
	...
	void rootCommentListKeepsDeletedParentOnlyWhenActiveReplyExists() {
		...
		PageResult<CommentListItemResult> response = commentQueryService.queryRootCommentPage(
	...
	void replyListExcludesDeletedReplies() {
		...
		PageResult<CommentListItemResult> response = commentQueryService.queryReplyPage(
