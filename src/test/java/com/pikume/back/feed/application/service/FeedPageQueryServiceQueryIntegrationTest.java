...
class FeedPageQueryServiceQueryIntegrationTest extends AbstractJpaQueryCountIntegrationTest {

	@Autowired
	private FeedPageQueryService feedPageQueryService;

	@Autowired
	private FeedDetailQueryService feedDetailQueryService;
	...
	void feedCursorQueryCountStaysBounded() {
		long oneItemQueries = measurePreparedStatements(() ->
				feedPageQueryService.queryPage(new FeedCursorRequest(null, 1), viewer.getId()));
		long threeItemQueries = measurePreparedStatements(() ->
				feedPageQueryService.queryPage(new FeedCursorRequest(null, 3), viewer.getId()));
	...
	void firstPagePrioritizesNotConsumedFriendBucket() {
		FeedCursorPage<FeedDiaryResult> page = feedPageQueryService.queryPage(
	...
	void nextCursorContinuesIntoNextBucket() {
		FeedCursorPage<FeedDiaryResult> firstPage = feedPageQueryService.queryPage(
				new FeedCursorRequest(null, 4),
				viewer.getId());

		FeedCursorPage<FeedDiaryResult> secondPage = feedPageQueryService.queryPage(
	...
	void feedCommentCountExcludesDeletedComments() {
		...
		FeedDiaryResult detail = feedDetailQueryService.queryDetail(publicHigh.getId(), viewer.getId());
		FeedCursorPage<FeedDiaryResult> latestPage = feedPageQueryService.queryPage(
				new FeedCursorRequest(null, 7, FeedSortMode.LATEST),
				viewer.getId());
