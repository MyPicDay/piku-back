...
public class DiaryController {
    ...
    @Autowired
    public DiaryController(DiaryService diaryservice , FileUtil fileUtil) {
    ...
    public ResponseEntity<String> createDiary(@ModelAttribute DiaryDTO diaryDTO, @AuthenticationPrincipal CustomUserDetails userDetails) {
        log.info(userDetails.getId() + "님 일기와 " + diaryDTO.getPhotos() +" 등록 요청");
        boolean isSaved = diaryservice.createDiary(diaryDTO, diaryDTO.getPhotos(), userDetails.getId());

        if (isSaved) {
            log.info(userDetails.getId() + "님 일기,사진 등록 성공");
    ...
    @GetMapping("/{diaryId}")
    public ResponseEntity<ResponseDTO> getDiaryWithPhotos(@PathVariable Integer diaryId) {
