...
public class JwtTokenProvider {
	...
	public String generateAccessToken(String userId) {
		log.debug("event=access_token_generation_requested");
		...
		log.debug("event=access_token_generated expiresAt={}", expiry);
		return Jwts.builder()
				.setClaims(claims)
				.setIssuedAt(now)
	...
	public String getUserIdFromToken(String token) {
		...
		log.debug("event=jwt_subject_parsed");
		return userId;
	}
	...
	public boolean validateToken(String token) {
		try {
			...
			log.debug("event=jwt_validation_failed reason={}", exception.getClass().getSimpleName());
			return false;
		}
	}
