...
public class AuthUserResponseMapper {
	...
	public UserInfo toDisplayUserInfo(LoginResult.UserInfo userInfo) {
		...
		return new UserInfo(
				userInfo.id(),
				userInfo.nickname(),
				resolveAvatarUrl(userInfo.avatarReference()), userInfo.profileSetupStatus());
	}
	...
	public UserInfo toDisplayUserInfo(UserPrincipal userDetails) {
		...
		return new UserInfo(
				...
				resolveAvatarUrl(userDetails.getAvatarReference()), userDetails.getProfileSetupStatus());
	}
